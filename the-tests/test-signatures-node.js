...
const { makeTree } = require('./dynamic-generative/make-tree.js')(getRandomInt);

const {
  treeAsArguments, noMergetreeType, mergeTreeType, testNumToStr, kKeyToXArgs, badTreeType, kKeyToXTypes,
} = require('./dynamic-generative/change-tree.js')(getRandomInt);
...
function reflectTree(tree_str, show_data) {
  ...
  const test_no_merge = type_czech.valid(the_arguments, no_merge_type);
...
function mutliNumTypeToSingleNum(tree_str, show_data) {
  ...
  const test_no_merge = type_czech.valid(the_arguments, merge_type);
  return test_no_merge;
}
...
function numberArgToString(tree_str, the_values, show_data) {
  ...
  if (bad_arguments) {
    ...
    const num_to_str_value_error = type_czech.valid(bad_arguments, merge_type);
...
function keyRenameKtoXinArgs(tree_str, the_keys, show_data) {
  ...
  if (bad_arguments) {
    ...
    const merge_type = noMergetreeType(tree_str);
    const key_k_now_x_error = type_czech.valid(bad_arguments, merge_type);
...
function numberTypeToStringType(tree_str, the_values, show_data) {
  ...
  if (bad_types) {
    ...
    const the_arguments = treeAsArguments(tree_str);
    const num_to_str_type_error = type_czech.valid(the_arguments, bad_types);
...
function keyRenameKtoXinTypes(tree_str, the_keys, show_data) {
  ...
  if (merge_type) {
    ...
    const key_k_now_x_error = type_czech.valid(the_arguments, merge_type);
    if (key_k_now_x_error === '') {
...
generativeTest(12, 'show-data');

for (let i = 0; i < 333; i += 1) {
  generativeTest(i);
}

// test the above to make sure they have errors !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
