...
let passed_typeCzechConstructor = 0;
let failed_typeCzechConstructor = 0;


_typeCzechConstructor_26001(26001);

total_fails += failed_typeCzechConstructor;
console.log('_typeCzechConstructor failed tests', failed_typeCzechConstructor);

total_checks += passed_typeCzechConstructor;
console.log('_typeCzechConstructor passed tests', passed_typeCzechConstructor);

function _typeCzechConstructor_26001(error_number) {
  const error_id = errorLabel(error_number);

  const type_czech_one_param = TypeCzech('LOG-ERRORS');
  if (type_czech_one_param.is_active) {
    passed_typeCzechConstructor += 1;
  } else {
    failed_typeCzechConstructor += 1;
    console.log(`${error_id} TypeCzech('LOG-ERRORS')`);
  }

  const type_czech_two_param = TypeCzech('LOG-ERRORS', 'CONSOLE-COUNT');
  ...
  const type_czech_array_param = TypeCzech(['LOG-ERRORS', 'CONSOLE-COUNT']);
